...
struct LiteralNode : public Node{
    Token value;
    explicit LiteralNode(Token t) : value(std::move(t)) {}
};

struct IdentifierNode : public Node{
    Token identifier;
    explicit IdentifierNode(Token t) : identifier(std::move(t)){}
};
...
struct UnaryOpNode : public Node{
    ...
    UnaryOpNode(Token o, std::unique_ptr<Node> expr) : op(std::move(o)), operand(std::move(expr)){}
};
...
struct BinaryOpNode : public Node {
    ...
    BinaryOpNode(std::unique_ptr<Node> l, Token o, std::unique_ptr<Node> r) : left(std::move(l)), op(std::move(o)), right(std::move(r)){}
};
...
struct FunctionCallNode : public Node {
    ...
    explicit FunctionCallNode(Token name) : functionName(std::move(name)){}
};
...
struct ProgramNode : public Node {
    ...
    ProgramNode(Token n, StmtPtr c, StmtPtr v, StmtPtr t, std::vector<NodePtr> subs, StmtPtr m) : name(std::move(n)), consts(std::move(c)), vars(std::move(v)), types(std::move(t)), subprograms(std::move(subs)), mainBlock(std::move(m)) {}
};

// struct ProgramNode : public Node {
//     Token programName;
//     std::vector<std::unique_ptr<Node>> declarations;
//     std::vector<std::unique_ptr<Node>> subprograms;
//     std::unique_ptr<Node> block;
// };

struct ConstSectionNode : public StmtNode {
    std::vector<std::pair<Token, ExprPtr>> entries;
    ConstSectionNode(std::vector<std::pair<Token, ExprPtr>> e) : entries(std::move(e)) {}
};
...
private: 
    ...
    std::unique_ptr<ProgramNode> program(){
        expect(TokenType::PROGRAM, "Esperado 'program'");
        Token progName = current();
        expect(TokenType::IDENTIFIER, "Esperado identificador do programa");
        expect(TokenType::SEMICOLON, "Esperado ';' após o nome do programa");

        // auto progNode = std::make_unique<ProgramNode>();
        // progNode->programName = progName;

        auto consts = declaracoes_const();
        auto vars   = declaracoes_var();
        auto types  = declaracoes_type();

        // if (match(TokenType::LABEL)) progNode->declarations.push_back(declaracoes_label());
        // if (match(TokenType::CONST)) progNode->declarations.push_back(declaracoes_const());
        // if (match(TokenType::TYPE)) progNode->declarations.push_back(declaracoes_type());
        // if (match(TokenType::VAR)) progNode->declarations.push_back(declaracoes_var());
        
        std::vector<NodePtr> subs =  declaracoes_func_procedure(); // procedures e functions
    

        // progNode->block = bloco();
        expect(TokenType::DOT, "Esperado '.' ao final do programa");
        return progNode;
    }
    ...
    std::unique_ptr<AssignNode> atribuicao(){
        auto node = std::make_unique<AssignNode>();
        node->indentifier = current();
        expect(TokenType::IDENTIFIER, "Esperado identificador");
        expect(TokenType::ASSIGN, "Esperado ':='"); //provavelmente n necessario por estar chamando atribuicao que um token de assign é encontrado
        node->expression = expressao();
        return node;
    }
    ...
    // std::unique_ptr<BinaryOpNode> expressao(){
    ...
    std::unique_ptr<Node> expressao(){
        auto left_node = termo();

        while(current().type == TokenType::PLUS || current().type == TokenType::MINUS){
            Token op = advance();
            auto right_node = termo();
            left_node = std::make_unique<BinaryOpNode>(std::move(left_node), op, std::move(right_node));
        }
        return left_node;
    }
    ...
    // std::unique_ptr<BinaryOpNode> termo(){
    //     auto node = std::make_unique<BinaryOpNode>();
    //     node->left =  fator();
    ...
    std::unique_ptr<Node> termo(){
        auto left_node = fator();

        while(current().type == TokenType::MULTIPLY || current().type == TokenType::DIVIDE || current().type == TokenType::DIV){
            Token op = advance();
            auto right_node = fator();
            left_node = std::make_unique<BinaryOpNode>(std::move(left_node), op, std::move(right_node));
        }
        return left_node;
    }
    ...
    std::unique_ptr<Node> fator(){
        if(match(TokenType::NOT)){
            ...
            // advance();
            return expr;
        }else if(current().type == TokenType::INT_LIT || current().type ==TokenType::REAL_LIT){
            ...
            if(lookahead().type == TokenType::OPEN_PAREN){
                ...
                auto func_call_node = std::make_unique<FunctionCallNode>(func_name);
                if(current().type != TokenType::CLOSE_PAREN){
                    do{
                        func_call_node->arguments.push_back(expressao());
                    }while(match(TokenType::COMMA));
                }
                expect(TokenType::CLOSE_PAREN, "Esperado ')' no final da chamada de função");
    ...
    //deve retornar uma array de funçoes
